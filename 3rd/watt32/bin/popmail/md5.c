...
static void Transform (DWORD[4], BYTE [64]);
static void Encode    (BYTE *, DWORD *, DWORD);
static void Decode    (DWORD *, BYTE *, DWORD);
...
void MD5Update (MD5_CTX *context,     /* context */
                ...
                DWORD    inputLen)    /* length of input block */
{
 ...
 */
void MD5Final (BYTE    digest[16],  /* message digest */
 ...
 */
static void Transform (DWORD state[4], BYTE block[64])
{
  DWORD a = state[0];
  DWORD b = state[1];
  DWORD c = state[2];
