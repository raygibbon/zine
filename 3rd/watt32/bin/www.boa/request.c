...
void get_request (void)
{
  ...
  int    fd, addrlen = sizeof(remote_addr);
  request *conn;                              /* connection */

  fd = accept (server_s, (struct sockaddr*)&remote_addr, &addrlen);
  if (fd < 0)
  {
    if (errno != EAGAIN && errno != EWOULDBLOCK) /* accept error */
       log_printf ("accept: %s\n", strerror(errno));
    if (errno == EINTR)
       sigint (-1);
    return;
  ...
  if ((setsockopt(fd, SOL_SOCKET,SO_REUSEADDR,(void*)&true,sizeof(true))) < 0)
     die (NO_SETSOCKOPT);
   ...
   */
  conn->remote_ip_addr = strdup (inet_ntoa(remote_addr.sin_addr));
...
void process_requests (void)
{
  ...
  while (current)
  {
    switch (current->status)    /* state for this client */
    {
      case READ_HEADER:
      case ONE_CR:
      case ONE_LF:
...
int process_logline (request *req)
{
  ...
  {
    case 3:                                           /* HTTP/1.0 */
         if (!strcmp(req->http_version,"0.9"))
            req->simple = 1;
         break;
    case 2:                                           /* HTTP/0.9 */
         req->simple = 1;
    ...
    default:
         ...
         log_printf ("malformed request: \"%s\"\n", req->logline);
         return (0);
  }
  ...
  *value++ = '\0';   /* overwrite the : */
  ...
  if (!memcmp(line,"ACCEPT",7))
     add_accept_header (req, value);
       ...
       req->keepalive = (!strnicmp(value,"Keep-Alive",10) ? 3 : 2);
