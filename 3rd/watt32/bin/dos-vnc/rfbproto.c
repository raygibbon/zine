...
unsigned long BGR233ToPixel[256];
...
void CopyBGR233ToScreen (CARD8 *buf, int x, int y, int width, int height)
{
}
...
ConnectToRFBServer(const char *hostname, int port)
{
    ...
    if (!StringToIPAddr(hostname, &host)) {
        fprintf(logFile,"Couldn't convert '%s' to host address\n",
                hostname);
        return False;
    }
    ...
    if (rfbsock < 0) {
        fprintf(logFile,"Unable to connect to VNC server\n");
        return False;
    }
...
Bool
InitialiseRFBConnection(sockType sock)
{
    ...
    if (!ReadExact(sock, pv, sz_rfbProtocolVersionMsg))
        return False;
    ...
    if (sscanf(pv,rfbProtocolVersionFormat,&major,&minor) != 2) {
        fprintf(logFile,"Not a valid VNC server\n");
        return False;
    }

    fprintf(logFile,"VNC server supports protocol version %d.%d "
            "(viewer %d.%d)\n",
    ...
    case rfbConnFailed:
        if (!ReadExact(sock, (char *)&reasonLen, 4))
            return False;
        ...
        fprintf(logFile,"VNC connection failed: %.*s\n",
                (int)reasonLen, reason);
        return False;

    case rfbNoAuth:
        fprintf(logFile,"No authentication needed\n");
        break;
    ...
    case rfbVncAuth:
        ...
        if (!ReadExact(sock, (char *)challenge, CHALLENGESIZE))
            return False;
        ...
        if (passwdFile) {
            ...
            if (strlen(passwd) == 0) {
                fprintf(logFile,"Reading password failed\n");
                return False;
            }
            if (strlen(passwd) > 8) {
                passwd[8] = '\0';
        ...
        fprintf(logFile,"Encrypting password...\n");
        ...
        fprintf(logFile,"Sending challenge to server...\n");

        if (!WriteExact(sock, challenge, CHALLENGESIZE))
            return False;

        fprintf(logFile,"Waiting for authentication result...\n");

        if (!ReadExact(sock, (char *)&authResult, 4))
            return False;
        ...
        switch (authResult) {
        case rfbVncAuthOK:
            fprintf(logFile,"VNC authentication succeeded\n");
            break;
        case rfbVncAuthFailed:
            fprintf(logFile,"VNC authentication failed\n");
            return False;
        case rfbVncAuthTooMany:
            fprintf(logFile,"VNC authentication failed - too many tries\n");
            return False;
        default:
            fprintf(logFile,"Unknown VNC authentication result: %d\n",
                        (int)authResult);
            return False;
        }
        break;
    ...
    {
        fprintf(logFile,
                "region requested is outside server's framebuffer\n");
        return False;
    }
    if (updateRequestW == 0)
    ...
    fprintf(logFile,"Desktop name \"%s\"\n",desktopName);
    fprintf(logFile,"Desktop size is %dx%d pixels\n",
    ...
    fprintf(logFile,"VNC server default format:\n");
...
HandleRFBServerMessage()
{
    ...
    case rfbSetColourMapEntries:
    {
        ...
        for (i = 0; i < msg.scme.nColours; i++) {
            if (!ReadExact(rfbsock, (char *)rgb, 6))
                return False;
            xc.r = Swap16IfLE(rgb[0]) >> 10;
            xc.g = Swap16IfLE(rgb[1]) >> 10;
            xc.b = Swap16IfLE(rgb[2]) >> 10;
            set_color(msg.scme.firstColour+i,&xc);
            pal[msg.scme.firstColour+i]=xc;
        }
    ...
    case rfbFramebufferUpdate:
    {
        ...
        for (i = 0; i < msg.fu.nRects; i++) {
            ...
            {
                fprintf(logFile,"%s: rect too large: %dx%d at (%d, %d)\n",
                       programName, rect.r.w, rect.r.h, rect.r.x, rect.r.y);
                return False;
            }

            if ((rect.r.h * rect.r.w) == 0) {
                fprintf(logFile,"%s: zero size rect - ignoring\n",programName);
                continue;
            }
...
#endif
                blit(scr,scr,cr.srcX,cr.srcY,rect.r.x,rect.r.y,
                        rect.r.w,rect.r.h);
            ...
            case rfbEncodingRRE:
            {
                ...
                case 8:
                    if (!ReadExact(rfbsock, (char *)&pix8, 1))
                        return False;
                        rectfill(scr,rect.r.x,rect.r.y,rect.r.x+rect.r.w-1,
                                rect.r.y+rect.r.h-1,
                    ...
                    for (j = 0; j < hdr.nSubrects; j++) {
                        ...
                        rectfill(scr,rect.r.x+subrect.x,
                                        rect.r.y+subrect.y,
                                        rect.r.x+subrect.x+rect.r.w-1,
                                        rect.r.y+subrect.y+rect.r.h-1,
                                (useBGR233 ? BGR233ToPixel[pix8] : pix8));
                    }
                    break;
                ...
                case 16:
                    if (!ReadExact(rfbsock, (char *)&pix16, 2))
                        ...
                        rectfill(scr,rect.r.x,rect.r.y,
                                        rect.r.x+rect.r.w-1,rect.r.y+rect.r.h-1,
                                        pix16);
            ...
            case rfbEncodingCoRRE:
            {
                ...
                case 8:
                    ...
                    for (j = 0; j < hdr.nSubrects; j++) {
                        ...
                        w = *ptr++;
                        h = *ptr++;
                        rectfill(scr,rect.r.x+x,rect.r.y+y,
                                        rect.r.x+x+w-1,rect.r.y+y+h-1,
                                 (useBGR233 ? BGR233ToPixel[pix8] : pix8));
                    }
                    break;
            ...
            default:
                fprintf(logFile,"%s: unknown rect encoding %d\n",programName,
                       (int)rect.encoding);
                return False;
            }
        }
    ...
    default:
        fprintf(logFile,"%s: unknown message type %d from VNC server\n",
        ...
        for (x = rx; x < rx+rw; x += 16) {                                    \
            ...
            if (subencoding & rfbHextileBackgroundSpecified)                  \
                if (!ReadExact(rfbsock, (char *)&bg, (bpp/8)))                \
                    return False;                                             \
            rectfill(scr,x,y,x+w-1,y+h-1,(bpp==8&&useBGR233?BGR233ToPixel[bg]:bg));       \
                                                                              \
            ...
            if (subencoding & rfbHextileSubrectsColoured) {                   \
                ...
                for (i = 0; i < nSubrects; i++) {                             \
                    ...
                    ptr++;                                                    \
            rectfill(scr,x+sx,y+sy,x+sx+sw-1,y+sy+sh-1,(bpp==8&&useBGR233?BGR233ToPixel[fg]:fg));       \
                }                                                             \
                                                                              \
            } else {                                                          \
    ...
    rfbPixelFormat *format;
{
    if (format->bitsPerPixel == 1) {
        fprintf(logFile,"Single bit per pixel.\n");
        fprintf(logFile,
                "%s significant bit in each byte is leftmost on the screen.\n",
                (format->bigEndian ? "Most" : "Least"));
    } else {
        fprintf(logFile,"%d bits per pixel.\n",format->bitsPerPixel);
        if (format->bitsPerPixel != 8) {
            fprintf(logFile,"%s significant byte first in each pixel.\n",
                    (format->bigEndian ? "Most" : "Least"));
        }
        if (format->trueColour) {
            fprintf(logFile,"True colour: max red %d green %d blue %d\n",
                    format->redMax, format->greenMax, format->blueMax);
            fprintf(logFile,"            shift red %d green %d blue %d\n",
                    format->redShift, format->greenShift, format->blueShift);
        } else {
            fprintf(logFile,"Uses a colour map (not true colour).\n");
        }
    }
    sleep(1);
}
