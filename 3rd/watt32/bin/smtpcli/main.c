...
**
**  Adapted for djgpp / Watt-32. G. Vanem - Sep 2002
*/
...
void usage(void)
{
    ...
    fprintf(stderr, "  -d, --debug            enable Watt-32 debug\n");
    fprintf(stderr, "\n");
    return;
}
...
int main(int argc, char **argv)
{
     ...
     *  Parse options
     */
    while ((c = getopt_long(argc, argv, ":s:f:r:e:c:S:P:MLvVh?d", options, NULL)) != EOF) {
        switch (c) {
            case 's':
                subject = optarg;
                break;
    ...
    if (gethostname(my_name, sizeof(my_name) - 1) < 0) {
        logit("gethostname: %s", errorstr(errno));
        exit(1);
    }
    if ((hp = gethostbyname(my_name)) == NULL) {
        logit("%s: unknown host\n", my_name);
        exit(1);
    }
     ...
     */
    if (from_addr == NULL) {
        if ((usr = getenv("USER")) == 0) {
            sprintf(buf, "userid-%d@%s", getuid(), my_name);
        } else {
            sprintf(buf, "%s@%s", usr, my_name);
        }
    ...
    if (hp->h_addrtype != AF_INET) {
        logit("unknown address family: %d", hp->h_addrtype);
        exit(1);
    }
    ...
    if ((s = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        logit("socket: %s", errorstr(errno));
        exit(1);
    }
    if (connect(s, (struct sockaddr *)&sin, sizeof(sin)) < 0) {
        logit("connect: %s", errorstr(errno));
        exit(1);
    }
    if ((r = dup(s)) < 0) {
        logit("dup: %s", errorstr(errno));
        exit(1);
    }
    if ((sfp = fdopen(s, "wb")) == 0) {
        logit("fdopen: %s", errorstr(errno));
        exit(1);
    }
    if ((rfp = fdopen(r, "rb")) == 0) {
        logit("fdopen: %s", errorstr(errno));
        exit(1);
    }
