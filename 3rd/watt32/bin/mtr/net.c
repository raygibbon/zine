...
#if defined(_WIN32) && !defined(WATT32) /* Raw sockets in Windows stinks */
  ttl = index + 1;
  setsockopt (sendsock, IPPROTO_IP, IP_TTL, (const char*)&ttl, sizeof(ttl));

  rv = sendto (sendsock, (const char*)icmp, packetsize - sizeof(struct IPHeader), 0,
...
int net_preopen() {
  ...
  {
    fprintf (stderr, "Failed to initialise Winsock. Error %u\n", GetLastError());
    return -1;
  }
  sendsock = socket (AF_INET, SOCK_RAW, IPPROTO_ICMP);
  if (sendsock == INVALID_SOCKET)
    return -1;
  recvsock = sendsock;    /* same socket for recv/xmit */
...
#else
  #warning IP_HDRINCL not available!!
#endif
...
void net_close()
{
  ...
  recvsock = sendsock = INVALID_SOCKET;
#else
  close (sendsock);
  close (recvsock);
#endif
