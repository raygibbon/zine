...
static long   current_year; /* approximation to current year */
...
static long totai (long year, long month, long mday)
{
  ...
  result = ((mday - 1) * 10 + 5 + 306 * month) / 10;
...
static long guesstai (long month, long mday)
{
  ...
  for (year = current_year - 1; year < current_year + 100; year++)
  {
...
{
  if (buf[0] != monthname[0] && buf[0] != monthname[0] - ' ')
     return (0);
  if (buf[1] != monthname[1] && buf[1] != monthname[1] - ' ')
     return (0);
  if (buf[2] != monthname[2] && buf[2] != monthname[2] - ' ')
     return (0);
  return (1);
}
...
int parse_line (struct ftpparse *fp, char *buf, int len)
{
  int  i, j, state;
  long size   = 0;
  long year   = 0;
  ...
  switch (*buf)
  {
    ...
    case '-':
         ...
         if (*buf == 'l') fp->flagislink  = fp->flagtryretr = 1;
             ...
             switch(state)
             {
               ...
               case 7:       /* have size, month, mday */
                    if (j-i == 4 && buf[i+1] == ':')
                    {
                      ...
                      initbase();
                      fp->mtime = base + guesstai(month,mday) +
                                  hour * 3600 + minute * 60;
                    }
