#ifndef __LIST_PARSE_H
#define __LIST_PARSE_H

/*
 * parse_line (&fp,buf,len) tries to parse one line of LIST output.
 *
 ...
 *
 * If parse_line() can't find a filename, it returns 0.
 *
 * If parse_line() can find a filename, it fills in fp and returns 1.
...
struct ftpparse {
       ...
       int    flagislink;  /* 1 if file/directory is a hard/soft link */
       int    sizetype;
...
extern int parse_line (struct ftpparse *fp, char *buf, int len);
