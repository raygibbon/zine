 ...
 */
#ifndef lint
char RCSid[] = "ttcp.c $Revision: 1.12 $";
#endif
...
Common options:\n\
	...
	-v	verbose: print more statistics\n\
        -d      set SO_DEBUG socket option and Watt-32 tracing\n\
...
void err(char *s);
void mes(char *s);
void pattern(char *a, int s);
void prep_timer(void);
void Delay(int us);
double read_timer(char *str, int len);
int Nread(int fd, void *buf, int len);
int Nwrite(int fd, void *buf, int len);
int mread(int fd, char *buf, unsigned len);
char *outfmt(double d);
...
#ifdef __WATCOMC__
        char io_buf[1024];
        setvbuf (stderr, io_buf, _IOLBF, sizeof(io_buf));
#endif
...
"ttcp: -b option ignored: SO_SNDBUF/SO_RCVBUF socket options not supported\n");
	...
	if (sinkmode) {      
		...
		register int cnt;
		if (trans)  {
                        while((cnt=fread(buf,1,buflen,stdin)) > 0 &&
			    Nwrite(fd,buf,cnt) == cnt)
				nbytes += cnt;
		}  else  {
			while((cnt=Nread(fd,buf,buflen)) > 0 &&
                            fwrite(buf,1,cnt,stdout) == cnt)
				nbytes += cnt;
		}
	}
        if(errno && errno != ENOSYS) err("IO");
	...
	for (; *cp; cp++)  {
		...
		case 'U':
			tvsub(&tdiff, &r1->ru_utime, &r0->ru_utime);
                        sprintf(outp,"%ld.%01ld", (long)tdiff.tv_sec, tdiff.tv_usec/100000);
			END(outp);
			break;
