...
int read_header (request *req)
{
  ...
  bytes = read_s (req->fd, buffer, MAX_HEADER_LENGTH);
  ...
  if (bytes == -1)
  {
    ...
    {
      boa_perror (req,errno,"header read");
      return (0);
    }
  }
    ...
    switch (req->status)
    {
      case READ_HEADER:
           if (*check == '\r')
                ...
                req->status = ONE_LF;
           else req->header_line[req->headerpos++] = *check;
           break;
  ...
  if (req->status == READ_BODY)
  {
    ...
    if (req->method == M_POST)
    {
      ...
      {
        boa_perror (req,errno,"tmpfile open");
        return (0);
      }
      req->post_file_name = strdup (tmpfilep);

      if (write(req->post_data_fd, check, buf_bytes_left) != buf_bytes_left)
      {
        boa_perror (req,errno,"tmpfile write");
        return (0);
      }
    }
    else
...
{ 
  ...
  int  bytes = read_s (req->fd, buffer, SOCKETBUF_SIZE);
  ...
  if (bytes <= 0)
  {
    ...
    if (errno == EBADF)  /* connection reset by peer */
       return (0);

    boa_perror (req,errno,"body read");
    return (0);
  }
