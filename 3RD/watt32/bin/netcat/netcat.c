...
#define MAXHOSTNAMELEN 256
  ...
  #define read_sock       read_s
  #define write_sock      write_s
  #define close_sock      close_s
  #define strcasecmp      stricmp
  extern int _watt_is_init;
#else
  #define read_sock       read
  #define write_sock      write
  #define close_sock      close
#endif

#define read_stdin(buf,size)   read(0,buf,size)
...
int comparehosts (struct host_poop *poop, struct hostent *hp);
HINF * gethostpoop (char *name, short unsigned int numeric);
USHORT getportpoop (char *pstring, unsigned int pnum);
USHORT nextport (char *block);
void loadports (char *block, short unsigned int lo, short unsigned int hi);
int doconnect (struct in_addr *rad, short unsigned int rp, struct in_addr *lad, short unsigned int lp);
int dolisten (struct in_addr *rad, short unsigned int rp, struct in_addr *lad, short unsigned int lp);
udptest (int fd, struct in_addr *where);
...
{
#if	0
  Tsk, tsk, word size assumption
  unsigned int s = (size + 4) & 0xfffffffc;	/* 4GB?! */
#endif
  unsigned int s = (size + 4) & ~3;
  char * p = malloc (s);
  if (p != NULL)
    memset (p, 0, s);
  else
...
/* wrap connect inside a timer, and hit it */
  ...
  if (rr == 0)
    return (nnetfd);
  close_sock (nnetfd);                  /* clean up junked socket FD!! */
  return (-1);
} /* doconnect */
...
/* fall here for TCP */
  ...
  if (setjmp (jbuf) == 0) {
    ...
    goto dol_tmo;		/* timeout */
  arm (0, 0);
  close_sock (nnetfd);          /* dump the old socket */
...
/* Set a temporary connect timeout, so packet filtration doesnt cause
   us to hang forever, and hit it */
    ...
    if (rr > 0)
      close_sock (rr);                  /* in case it *did* open */
    o_wait = 0;				/* reset it */
  ...
  if (rr == 1)				/* if write error, no UDP listener */
    return (fd);
  close_sock (fd);                      /* use it or lose it! */
  return (-1);
} /* udptest */
...
/* Ding!!  Something arrived, go check all the incoming hoppers, net first */
    if (FD_ISSET (fd, ding2)) {		/* net: ding! */
        rr = read_sock (fd, bigbuf_net, BIGSIZ);
	if (rr <= 0) {
...
/* okay, suck more stdin */
    if (FD_ISSET (0, ding2)) {          /* stdin: ding! */
        rr = read_stdin (bigbuf_in, BIGSIZ);
...
Debug (("wrote %d to stdout, errno %d", rr, errno))
	...
	if (o_interval)			/* in "slowly" mode ?? */
	  ...
	  rr = rzleft;
        rr = write_sock (fd, zp, rr);   /* one line, or the whole buffer */
	if (rr > 0) {
	  if (o_wfile)
...
#if defined(HAVE_BIND) && !defined(WATT32)
...
/* if no args given at all, get 'em from stdin, construct an argv, and hand
  ...
  if (argc == 1) {
    ...
    insaved = read_stdin (cp, BIGSIZ);     /* we're gonna fake fgets() here */
    if (insaved <= 0)
      bail ("wrong");
...
#endif /* G_S_H */
  if (o_wfile) {
    if (stage[0] == '-' && stage[1] == '\0')
         ofd = STDERR_FILENO;
    else ofd = open ((const char*)stage, O_WRONLY | O_CREAT | O_TRUNC, 0664);
...
/* if we're scanning at a "one -v" verbosity level, don't print refusals.
   Give it another -v if you want to see everything. */
	if ((Single || (o_verbose > 1)) || (errno != ECONNREFUSED))
      ...
      if (netfd >= 0)
         close_sock (netfd);            /* just in case we didn't already */
      if (o_interval)
