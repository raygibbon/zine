...
#define DIRSRV_PORT      2050       /* Server port used if not in srvtab */
#define PROSPERO_PORT    1525       /* Officially assigned prived port   */

#define MAX_PTXT_LEN     1250       /* Max length of struct ptext        */
...
#define MAX_DIR_LINESIZE (160+128)  /* Max linesize in directory */
#define MAX_FWD_DEPTH    20         /* Max fwd pointers to follow        */

/* Queuing priorities for datagrams */
#define RDGRAM_MAX_PRI   32765         /* Maximum user priority          */
#define RDGRAM_MIN_PRI  -32765         /* Maximum user priority          */
...
struct ptext {
       ...
       DWORD         mbz;              /* ZERO to catch runaway strings  */
...
/* Procedures in support.c */

int             wcmatch    (char *s, char *template);
char           *nlsindex   (char *s1,char *s2);
int             ul_insert  (struct vlink *ul, struct vdir *vd, struct vlink *p);
int             vl_insert  (struct vlink *vl, struct vdir *vd, int allow_conflict);
...
int             get_vdir(char *dhost, char *dfile, struct vdir *dir, int flags);
...
extern void Adebug (int lvl, char *fmt, ...);
