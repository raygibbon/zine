\documentstyle[11pt,twoside,a4]{report}
% New commands
%-------------
...
\Large Version 5\\
\vskip 1.5em
{\large Draft of 25 November 1994} \\
{\large Document Revision No. 0.4.0} \\
\end{center}
\vspace{.5in}
...
have not done much implementation work on the \lit{ID} field, pending
...
\lit{P\_PASSWORD}, and \lit{HANDLE}.\footnote{XXX \lit{HANDLE} actually
not yet implemented.}
...
\meta{authentication-data} is a Kerberos Version 5 authentication message
authenticating the principal to the Prospero server.  The server's
principal is always \lit{prospero}.  The ACL principal will be the
same as the client's Kerberos principal.  The ACL type will be
\lit{AUTHENT} \lit{KERBEROS}.
...
\begin{command}
  \commandsize \lit{OBJECT} \meta{hsoname-type} \meta{hsoname}
	\zoos\meta{object-version}\zooe \selectlines
\end{command}
...
or \meta{LF} was to be included in a component
...
The \meta{name-component} tokens in the \lit{LIST} command are the
...
For instance, specifying the name-component \lit{(Anne.*)} is equivalent to specifying the
wildcarded name-component \lit{Anne*}.

In addition to matching the component as a wildcard, a literal match
for the component will also happen.  For example, the \meta{name-component}
\lit{(Anne.*)} will always match a link whose literal name is
\lit{(Anne.*)}.  This special case guarantees that clients need not
...
The \lit{ATTRIBUTES} option indicates that the attributes associated
...
the link, except for attributes with the \lit{-} suffix.\footnote{
   ...
   \item[\lit{\#CACHED}] Return all attributes of precedence \lit{CACHED}.
...
specified \lit{\#ALL}.  It is not clear how useful this behavior is.
   \item[\lit{\#OBJECT}] Return all attributes of type \lit{OBJECT}.
This modifies the rest of the attribute request.  It cannot be
free-standing; \lit{\#OBJECT} by itself is undefined.
Note that
this will not work if the object does not reside on the same host as
the link.
   \item[\lit{\#ADDITIONAL}] Return all attributes of precedence \lit{ADDITIONAL}.
   \item[\lit{\#REPLACEMENT}] Return all attributes of precedence \lit{REPLACEMENT}.
   \item[\lit{\#LINK}] Return all attributes of precedence \lit{LINK}.
...
the \lit{LINK} response, such as \atr{host}, \atr{hsoname}, and
\atr{dest-exp}.
...
\begin{command}
  \lit{UNRESOLVED} \zoms\meta{name-component}\zome
\end{command}

The \meta{name-component}s must be a proper suffix of
the \meta{name-component}s sent as an argument to the \lit{LIST}
...
\begin{command}
  \lit{UNRESOLVED} \lit{murali} \lit{ROOT} \\
...
This request can be used to list the \lit{ACL} (access control list) for
the object specified in the previous \lit{OBJECT}
command.\footnote{Versions of Prospero Alpha.5.2 and earlier
required a preceding \lit{DIRECTORY} command,  not the \lit{OBJECT}
command.}
If the
object is a
\lit{DIRECTORY}, this request can be used to list the access control list
for a link within that directory.
The optional \meta{name-component} is required only if requesting the
\lit{ACL} for a link within the directory (option = \lit{LINK}).  It
is to be left out when requesting the \lit{ACL} for the object itself
(option = \lit{OBJECT}).\footnote{XXX This used to be in one of two
...
specify the option = \lit{DIRECTORY}, exactly as one currently
specifies the \lit{OBJECT} option.
The old way of retrieving information about an OBJECT's ACL was to
specify the \lit{OBJECT} option in the options field and replace the \meta{name-component} field
above with \meta{hsoname-type} \meta{hsoname} \zoos\meta{object-version}\zooe.}
...
There are four special \lit{NAMED} ACLs:\footnote{CHANGE: In previous
revisions of this Protocol manual, we distinguished \lit{NAMED} ACLs
from \lit{INCLUDED} ACLs.  This distinction no longer exists, and
\lit{INCLUDED} ACL retrieval was never
...
gone. ---swa, 11/25/94} \lit{SYSTEM}, \lit{MAINTENANCE},
\lit{DEFAULT}, and \lit{OVERRIDE}. 

The new \lit{CONTAINER} option is not yet fully deployed in our
...
\lit{LIST} command.  The special names are: \lit{\#OBJECT}, which is synonymous
...
\lit{EDIT-ACL}
 \ors \lit{LINK+}\meta{options} \meta{name-component} \\
	\hspace{.6in} \metaor \lit{OBJECT+}\meta{options} \ore	\\
...
specified in the previous \lit{OBJECT} command.
...
(\lit{NOSELF}).  For \lit{NAMED} ACLs, the additional operations
\lit{CREATE} and \lit{DESTROY} are available.
...
\lit{VIRTUAL+DIRECTORY} options.  It is unclear whether there will
...
\lit{SUCCESS} or error indicators are returned.
...
Upon success with the \lit{PHYSICAL} option, the server will return:
\begin{command}
...
This command is unimplemented.  Its form is not final; this section on
...
contact \lit{info-prospero@ISI.EDU}.
...
\begin{command}
  \lit{NOT-FOUND} \ors\lit{FILE} \metaor \lit{LINK} \metaor \lit{ACL} \metaor
...
\lit{info-prospero@ISI.EDU}) or start with a prefix registered for the
application or start with the two-character sequence \lit{X-}.
...
of three or more elements.  This is the same as the \atr{principal}
attribute on the owner's \lit{VS-DESCRIPTION} directory.
%The first element is an ACL
...
If there is a \atr{collation-order} attribute in addition, then
\atr{collation-order} is a major sort key and this is the minor key.
...
\item[\lit{INCLREAL}]	Same as above, but (for the \unix\  server) do not
include ``\lit{.}'' and ``\lit{..}''.
