...
/* Needed for compilation under OS/2 or MSDOS: */
...
#ifdef __HIGHC__
  #define SETMODE(f,m)  setmode(f,m)
#elif (defined(m) && (m) != 0)
  #define SETMODE(f,m)  setmode(fileno(f), m)
#else
  #define SETMODE(f,m)  ((void)0)
#endif

#if defined(__BORLANDC__) && !defined(off_t)
  #define off_t long
#endif
